...
public class AiffTagWriter {
    private static final JLogger LOG = JLoggers.get(AiffTagWriter.class, EalvaTagLog.MARKER);

    /**
     * Delete given {@link Tag} from file.
     */
    public void delete(Tag tag, FileChannel fc, String fileName) throws CannotWriteException {
        try {
            LOG.log(LogLevel.WARN, "%s Deleting tag from file", fileName);
            AiffTag existingTag = getExistingMetadata(fc, fileName);
            fc.position(0);
     ...
     */
    private AiffTag getExistingMetadata(FileChannel channel, String fileName)
    ...
    private void deleteTagChunkUsingSmallByteBufferSegments(AiffTag existingTag,
                                                            FileChannel channel,
                                                            long newLength,
                                                            long lengthTagChunk)
            throws IOException {
     ...
     * @throws IOException if something goes wrong
     */
    private void deleteTagChunkUsingChannelTransfer(AiffTag existingTag,
                                                    FileChannel channel,
                                                    long newLength)
            throws IOException {
        long read;
    ...
    public void write(Tag tag, FileChannel fc, String fileName) throws CannotWriteException {
