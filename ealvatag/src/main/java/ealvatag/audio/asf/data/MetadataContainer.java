...
import ealvatag.audio.asf.io.WriteableChunk;
...
public class MetadataContainer extends Chunk implements WriteableChunk {
    ...
    public final void addDescriptor(MetadataDescriptor toAdd) throws IllegalArgumentException {
        ...
        containerType.assertConstraints(toAdd.getName(), toAdd.getRawData(), toAdd.getType(), toAdd.getStreamNumber(), toAdd.getLanguageIndex());
        ...
        synchronized (perfPoint) {
            list = descriptors.get(perfPoint.setDescriptor(toAdd));
        }
        if (list == null) {
            list = new ArrayList<MetadataDescriptor>();
            descriptors.put(new DescriptorPointer(toAdd), list);
        } else {
            if (!list.isEmpty() && !containerType.isMultiValued()) {
    ...
    public long getCurrentAsfChunkSize() {
        ...
        for (MetadataDescriptor curr : getDescriptors()) {
            result += curr.getCurrentAsfSize(containerType);
        }
        return result;
    }
    ...
    public final List<MetadataDescriptor> getDescriptors() {
        ...
        for (List<MetadataDescriptor> curr : descriptors.values()) {
            result.addAll(curr);
        }
        return result;
    }
    ...
    public final List<MetadataDescriptor> getDescriptorsByName(String name) {
        assert name != null;
        List<MetadataDescriptor> result = new ArrayList<MetadataDescriptor>();
        Collection<List<MetadataDescriptor>> values = descriptors.values();
    ...
    public boolean isAddSupported(MetadataDescriptor descriptor) {
        ...
        if (result && !getContainerType().isMultiValued()) {
            synchronized (perfPoint) {
                List<MetadataDescriptor> list = descriptors.get(perfPoint.setDescriptor(descriptor));
                if (list != null) {
                    result = list.isEmpty();
                }
            }
    ...
    public final void removeDescriptorsByName(String name) {
        assert name != null;
        Iterator<List<MetadataDescriptor>> iterator = descriptors.values().iterator();
    ...
    private final static class DescriptorPointer {
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof DescriptorPointer && !result) {
                ...
                result = desc.getName().equals(other.getName());
                result &= desc.getLanguageIndex() == other.getLanguageIndex();
                result &= desc.getStreamNumber() == other.getStreamNumber();
            }
            return result;
        }
        ...
        public int hashCode() {
            int hashCode;
            hashCode = desc.getName().hashCode();
            hashCode = hashCode * 31 + desc.getLanguageIndex();
            hashCode = hashCode * 31 + desc.getStreamNumber();
            return hashCode;
        }
