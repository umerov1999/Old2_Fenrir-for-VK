...
public class MetadataDescriptor implements Comparable<MetadataDescriptor>, Cloneable {
     ...
     */
    static final BigInteger QWORD_MAXVALUE = new BigInteger("FFFFFFFFFFFFFFFF", 16);
    /**
     * Maximum value for WORD.
     */
    static final int WORD_MAXVALUE = 65535;
    /**
     * Logger instance.
     */
    private static final JLogger LOG = JLoggers.get(MetadataDescriptor.class, EalvaTagLog.MARKER);
    /**
     ...
     * Creates an Instance.
     *
     * @param type     The container type the values (the whole descriptor) is restricted to.
     * @param propName Name of the MetadataDescriptor.
     * @param propType Type of the metadata descriptor. See {@link #descriptorType}
     * @param stream   the number of the stream the descriptor refers to.
     * @param language the index of the language entry in a {@link LanguageList} this descriptor refers to.<br> <b>Consider</b>: No checks
     *                 performed if language entry exists.
     */
    ...
    BigInteger asNumber() {
        ...
        if (result == null) {
            ...
            for (int i = 0; i < copy.length; i++) {
                copy[i] = content[content.length - (i + 1)];
            }
    ...
    public MetadataDescriptor createCopy() {
        MetadataDescriptor result = new MetadataDescriptor(containerType,
                name,
                descriptorType,
                streamNumber,
                languageIndex);
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof MetadataDescriptor) {
            if (obj == this) {
                ...
                MetadataDescriptor other = (MetadataDescriptor) obj;
                result = other.getName().equals(getName()) && other.descriptorType == descriptorType &&
                        other.languageIndex == languageIndex && other.streamNumber == streamNumber &&
    ...
    public GUID getGuid() {
        GUID result = null;
        if (getType() == TYPE_GUID && content.length == GUID.GUID_LENGTH) {
            result = new GUID(content);
        }
        return result;
    }
    ...
    void setLanguageIndex(int language) {
        containerType.assertConstraints(name, content, descriptorType, streamNumber, language);
        languageIndex = language;
    }
    ...
    public long getNumber() {
        ...
        for (int i = 0; i < bytesNeeded; i++) {
            result |= (((long) content[i] & 0xFF) << (i * 8));
        }
        return result;
    }
    ...
    void setStreamNumber(int stream) {
        containerType.assertConstraints(name, content, descriptorType, stream, languageIndex);
        streamNumber = stream;
    }
    ...
    public String getString() {
        ...
        switch (getType()) {
            ...
            case TYPE_WORD:
                ...
                break;
            case TYPE_STRING:
                result = new String(content, StandardCharsets.UTF_16LE);
                break;
            default:
    ...
    public void setString(String value) throws IllegalArgumentException {
        try {
            ...
            throw new IllegalArgumentException("Value cannot be parsed as Number or is out of range (\"" + value +
                    "\")",
                    nfe);
        }
    }
     ...
     * @throws IllegalArgumentException if data is invalid for {@linkplain #getContainerType() container}.
     */
    public void setBinaryValue(byte[] data) throws IllegalArgumentException {
        containerType.assertConstraints(name,
                data,
                descriptorType,
                streamNumber,
                languageIndex);
    ...
    public void setBooleanValue(boolean value) {
        content = new byte[]{value ? (byte) 1 : 0};
     ...
     */
    void setQWordValue(BigInteger value) throws IllegalArgumentException {
        if (value == null) {
        ...
        if (valuesBytes.length <= 8) {
            for (int i = valuesBytes.length - 1; i >= 0; i--) {
                content[valuesBytes.length - (i + 1)] = valuesBytes[i];
            }
        } else {
            /*
    ...
    public void setQWordValue(long value) {
        if (value < 0) {
            throw new IllegalArgumentException(
                    "value out of range (0-" + QWORD_MAXVALUE + ")");
        }
        ...
        if (value == null) {
            ...
            if (getContainerType().isWithinValueRange(tmp.length)) {
                ...
                if (TagOptionSingleton.getInstance().isTruncateTextWithoutErrors()) {
                    // truncate the string
                    int copyBytes = (int) getContainerType().getMaximumDataLength().longValue();
                    content = new byte[copyBytes % 2 == 0 ? copyBytes : copyBytes - 1];
                    System.arraycopy(tmp, 0, content, 0, content.length);
                } else {
    ...
    public void setWordValue(int value) throws IllegalArgumentException {
        ...
        content = Utils.getBytes(value, 2);
     ...
     */
    int writeInto(OutputStream out, ContainerType contType) throws IOException {
        int size = getCurrentAsfSize(contType);
        /*
