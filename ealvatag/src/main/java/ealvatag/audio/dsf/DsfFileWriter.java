...
public class DsfFileWriter extends AudioFileWriter2 {
    protected void writeTag(TagFieldContainer tag, FileChannel fc, String fileName) throws CannotWriteException {
        try {
     ...
     */
    @Override
    protected void deleteTag(Tag tag, FileChannel fc, String fileName) throws CannotWriteException {
        try {
            DsdChunk dsd = DsdChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH));
            if (dsd != null) {
                if (dsd.getMetadataOffset() > 0) {
