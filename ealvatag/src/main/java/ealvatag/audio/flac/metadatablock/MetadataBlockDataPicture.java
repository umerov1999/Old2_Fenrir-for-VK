 ...
 * n*8 	The MIME type string, in printable ASCII characters 0x20-0x7e. The MIME type may also be -- to signify that
 * the data part is a URL of the picture instead of the picture data itself.
...
public class MetadataBlockDataPicture
    ...
    // Logger Object
    public static JLogger LOG = JLoggers.get(MetadataBlockDataPicture.class, EalvaTagLog.MARKER);
    ...
    public MetadataBlockDataPicture(MetadataBlockHeader header, FileChannel fc)
        ...
        if (bytesRead < header.getDataLength()) {
            throw new IOException("Unable to read required number of databytes read:" + bytesRead + ":required:" +
    ...
    private void initFromByteBuffer(ByteBuffer rawdata) throws IOException, InvalidFrameException {
        ...
        if (pictureType >= PictureTypes.getInstanceOf().getSize()) {
            throw new InvalidFrameException("PictureType was:" + pictureType + "but the maximum allowed is " +
                    (PictureTypes.getInstanceOf().getSize() - 1));
        }
        ...
        LOG.log(TRACE, "Read image:%s", this);
    }
     ...
     * @throws java.io.UnsupportedEncodingException Most tag data represents text. In some cases the underlying implementation will need to
     *                                              convert the text data in java to a specific charset encoding. In these cases an {@link
