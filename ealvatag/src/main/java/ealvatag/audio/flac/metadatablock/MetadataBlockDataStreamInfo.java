 ...
 * headers to 655350Hz. Also,
 * a value of 0 is invalid.
 * 3 	(number of channels)-1. FLAC supports from 1 to 8 channels
 * 5 	(bits per sample)-1. FLAC supports from 4 to 32 bits per sample. Currently the reference encoder and decoders
 * only support up to 24 bits per sample.
 ...
 * NOTES
 * * FLAC specifies a minimum block size of 16 and a maximum block size of 65535, meaning the bit patterns
...
public class MetadataBlockDataStreamInfo
    ...
    public MetadataBlockDataStreamInfo(MetadataBlockHeader header, FileChannel fc) throws IOException {
        ...
        if (bytesRead < header.getDataLength()) {
            throw new IOException("Unable to read required number of bytes, read:" + bytesRead + ":required:" +
    ...
    public String toString() {
        ...
        return "MinBlockSize:" + minBlockSize + "MaxBlockSize:" + maxBlockSize + "MinFrameSize:" + minFrameSize +
                "MaxFrameSize:" + maxFrameSize + "SampleRateTotal:" + samplingRate + "SampleRatePerChannel:" +
                samplingRatePerChannel + ":Channel number:" + noOfChannels + ":Bits per sample: " + bitsPerSample +
                ":TotalNumberOfSamples: " + noOfSamples + ":Length: " + trackLength;
