...
public class Mp4IlstBox {
    ...
    public Mp4IlstBox(Mp4BoxHeader ilstBoxHeader,
                      BufferedSource bufferedSource,
                      Mp4Tag mp4Tag,
                      boolean ignoreArtwork) throws IOException, CannotReadException {
        ...
        while (dataSize >= Mp4BoxHeader.HEADER_LENGTH) {
            ...
            int dataLength = childHeader.getDataLength();
            if (dataLength > 0) {        //Header with no data #JAUDIOTAGGER-463
                if (Mp4TagReverseDnsField.IDENTIFIER.equals(childHeader.getId())) {  //Reverse Dns Atom
                    ...
                    boolean isDataIdentifier = Mp4DataBox.IDENTIFIER.equals(Buffers.peekString(bufferedSource,
    ...
    private void handleReverseDns(Mp4Tag tag,
                                  Mp4BoxHeader header,
    ...
    private void handleArtwork(Mp4Tag tag,
                               Mp4BoxHeader header,
                               ByteBuffer raw,
                               Mp4FieldType fieldType) throws UnsupportedEncodingException {
    ...
    private boolean isArtworkField(Mp4BoxHeader header, Mp4FieldType fieldType) {
