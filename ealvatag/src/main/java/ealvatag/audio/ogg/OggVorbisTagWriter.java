...
public class OggVorbisTagWriter {
    private static final JLogger LOG = JLoggers.get(OggVorbisTagWriter.class, EalvaTagLog.MARKER);
    ...
    public void write(TagFieldContainer tag, RandomAccessFile raf, RandomAccessFile rafTemp)
        ...
        LOG.log(DEBUG, "Starting to write file:");

        //1st Page:Identification Header
        LOG.log(DEBUG, "Read 1st Page:identificationHeader:");
        OggPageHeader pageHeader = OggPageHeader.read(raf);
        raf.seek(pageHeader.getStartByte());

        //Write 1st page (unchanged) and place writer pointer at end of data
        rafTemp.getChannel()
                .transferFrom(raf.getChannel(),
                        0,
                        pageHeader.getPageLength() + OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH +
                                pageHeader.getSegmentTable().length);
        rafTemp.skipBytes(pageHeader.getPageLength() + OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH +
                pageHeader.getSegmentTable().length);
        LOG.log(DEBUG, "Written identificationHeader:");
        ...
        LOG.log(DEBUG, "Read 2nd Page:comment and setup and possibly audio:Header finishes at file position:" +
        ...
        int newSecondPageDataLength =
                vorbisHeaderSizes.getSetupHeaderSize() + newCommentLength + vorbisHeaderSizes.getExtraPacketDataSize();
        LOG.log(DEBUG, "Old 2nd Page no of packets: " + secondPageHeader.getPacketList().size());
        LOG.log(DEBUG, "Old 2nd Page size: " + secondPageHeader.getPageLength());
        LOG.log(DEBUG, "Old last packet incomplete: " + secondPageHeader.isLastPacketIncomplete());
        LOG.log(DEBUG, "Setup Header Size: " + vorbisHeaderSizes.getSetupHeaderSize());
        LOG.log(DEBUG, "Extra Packets: " + vorbisHeaderSizes.getExtraPacketList().size());
        LOG.log(DEBUG, "Extra Packet Data Size: " + vorbisHeaderSizes.getExtraPacketDataSize());
        LOG.log(DEBUG, "Old comment: " + vorbisHeaderSizes.getCommentHeaderSize());
        LOG.log(DEBUG, "New comment: " + newCommentLength);
        LOG.log(DEBUG, "New Page Data Size: " + newSecondPageDataLength);
        //Second Page containing new vorbis, setup and possibly some extra packets can fit on one page
        if (isCommentAndSetupHeaderFitsOnASinglePage(newCommentLength,
                vorbisHeaderSizes.getSetupHeaderSize(),
                vorbisHeaderSizes.getExtraPacketList())) {
            ...
            if ((secondPageHeader.getPageLength() < OggPageHeader.MAXIMUM_PAGE_DATA_SIZE) &&
                ...
                LOG.log(DEBUG, "Header and Setup remain on single page:");
                replaceSecondPageOnly(vorbisHeaderSizes,
                        newCommentLength,
            ...
            //Original 2nd page spanned multiple pages so more work to do
                ...
                LOG.log(DEBUG, "Header and Setup now on single page:");
                replaceSecondPageAndRenumberPageSeqs(vorbisHeaderSizes,
                        newCommentLength,
                        newSecondPageDataLength,
                        secondPageHeader,
                        newComment,
        ...
        //Bit more complicated, have to create more than one new page and renumber subsequent audio
            ...
            LOG.log(DEBUG, "Header and Setup with shift audio:");
            replacePagesAndRenumberPageSeqs(vorbisHeaderSizes,
                    newCommentLength,
                    secondPageHeader,
                    newComment,
                    raf,
    ...
    private ByteBuffer startCreateBasicSecondPage(
        ...
        LOG.log(DEBUG, "New second page header length:" + newSecondPageHeaderLength);
        LOG.log(DEBUG, "No of segments:" + segmentTable.length);
    ...
    private void replaceSecondPageOnly(
        ...
        LOG.log(DEBUG, "WriteOgg Type 1");
        ByteBuffer secondPageBuffer = startCreateBasicSecondPage(vorbisHeaderSizes,
                newCommentLength,
                newSecondPageLength,
                secondPageHeader,
                newComment);
        ...
        rafTemp.getChannel()
                .transferFrom(raf.getChannel(), rafTemp.getFilePointer(), raf.length() - raf.getFilePointer());
    }
    ...
    private void replaceSecondPageAndRenumberPageSeqs(OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
            ...
            throws IOException, CannotReadException, CannotWriteException {
        LOG.log(DEBUG, "WriteOgg Type 2");
        ByteBuffer secondPageBuffer = startCreateBasicSecondPage(originalHeaderSizes,
                newCommentLength,
                newSecondPageLength,
                secondPageHeader,
                newComment);
        ...
        byte[] setupHeaderData =
                reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(originalHeaderSizes
                                .getSetupHeaderStartPosition(),
                        raf);
        LOG.log(DEBUG, setupHeaderData.length + ":" + secondPageBuffer.position() + ":" + secondPageBuffer.capacity());
    ...
    private void replacePagesAndRenumberPageSeqs(OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes,
        ...
        LOG.log(DEBUG, "Comment requires:" + noOfCompletePagesNeededForComment + " complete pages");
        ...
        LOG.log(DEBUG, "Last comment packet size:" + lastPageCommentPacketSize);

        //End of comment and setup header cannot fit on the last page
        if (!isCommentAndSetupHeaderFitsOnASinglePage(lastPageCommentPacketSize,
                originalHeaderSizes.getSetupHeaderSize(),
                originalHeaderSizes.getExtraPacketList())) {
            LOG.log(DEBUG, "WriteOgg Type 3");
            ...
            //Write the last part of comment only (its possible it might be the only comment)
            {
                ...
                LOG.log(DEBUG, "Writing Last Comment Page " + pageSequence + " to file");
                pageSequence++;
                calculateChecksumOverPage(pageBuffer);
                rafTemp.getChannel().write(pageBuffer);
            }
            ...
            {
                byte[] segmentTable = createSegmentTable(originalHeaderSizes.getSetupHeaderSize(),
                        originalHeaderSizes.getExtraPacketList());
                int pageHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
                byte[] setupHeaderData =
                        reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(originalHeaderSizes
                ...
                pageBuffer.putInt(OggPageHeader.FIELD_PAGE_SEQUENCE_NO_POS, pageSequence);
                //pageBuffer.put(OggPageHeader.FIELD_HEADER_TYPE_FLAG_POS, OggPageHeader.HeaderTypeFlag
                // .CONTINUED_PACKET.getFileValue());
                LOG.log(DEBUG, "Writing Setup Header and packets Page " + pageSequence + " to file");
            ...
            LOG.log(DEBUG, "WriteOgg Type 4");

            //Create last header page
            int newSecondPageDataLength = originalHeaderSizes.getSetupHeaderSize() + lastPageCommentPacketSize +
                    originalHeaderSizes.getExtraPacketDataSize();
            newComment.position(newCommentOffset);
            ...
            lastHeaderBuffer.put(OggPageHeader.FIELD_HEADER_TYPE_FLAG_POS,
                    OggPageHeader.HeaderTypeFlag.CONTINUED_PACKET.getFileValue());
    ...
    public void writeRemainingPages(int pageSequence, RandomAccessFile raf, RandomAccessFile rafTemp)
        ...
        while (bb.hasRemaining()) {
            ...
            //putInt method
            ByteBuffer nextPageHeaderBuffer =
                    ByteBuffer.allocate(nextPage.getRawHeaderData().length + nextPage.getPageLength());
        ...
        if ((raf.length() - startAudio) != ((rafTemp.length() + bytesToDiscard) - startAudioWritten)) {
            throw new CannotWriteException("File written counts don't match, file not written:"
                    ...
                    + ":newAudioLength:" +
                    ((rafTemp.length() + bytesToDiscard) - startAudioWritten)
     ...
     * @param setupHeaderLength The length of Setup Header, zero if comment String extends over multiple pages and this is not the last
     *                          page.
     * @param extraPackets      If there are packets immediately after setup header in same page, they need including in the segment table
    ...
    private byte[] createSegmentTable(int newCommentLength,
        ...
        LOG.log(DEBUG, "Create SegmentTable CommentLength:" + newCommentLength + ":SetupHeaderLength:" + setupHeaderLength);
        ...
        LOG.log(DEBUG, "Created " + newStart.length + " segments for header");
        LOG.log(DEBUG, "Created " + restShouldBe.length + " segments for setup");
        ...
        try {
            ...
            if (extraPackets.size() > 0) {
                ...
                LOG.log(DEBUG, "Creating segments for " + extraPackets.size() + " packets");
     ...
     *
     * @param setupHeaderLength The length of Setup Header, zero if comment String extends over multiple pages and this is not the last page.
     * @param extraPackets      If there are packets immediately after setup header in same page, they need including in the segment table
     * @return new segment table.
     */
     ...
     *
     * @param length     Size of the page which should be represented as 255 byte packets.
     * @param quitStream If true and a length is a multiple of 255 we need another segment table entry with the value of 0. Else it's the last
     *                   stream of the table which is already ended.
    ...
    private byte[] createSegments(int length, boolean quitStream) {
        LOG.log(DEBUG, "Create Segments for length:" + length + ":QuitStream:" + quitStream);
     ...
     * @return true if there is enough room to fit the comment and the setup headers on one page taking into account the maximum no of
     * segments allowed per page and zero lacing values.
     */
    private boolean isCommentAndSetupHeaderFitsOnASinglePage(int commentLength,
        ...
        LOG.log(DEBUG, "Require:" + totalDataSize + " segments for comment");
        ...
        LOG.log(DEBUG, "Require:" + totalDataSize + " segments for comment plus setup");
        ...
        LOG.log(DEBUG, "Total No Of Segment If New Comment And Header Put On One Page:" + totalDataSize);
