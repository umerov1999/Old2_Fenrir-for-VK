...
public class VorbisIdentificationHeader implements VorbisHeader {
    ...
    private static final JLogger LOG = JLoggers.get(VorbisIdentificationHeader.class, EalvaTagLog.MARKER);
    ...
    public void decodeHeader(byte[] b) {
        ...
        if (packetType == VorbisPacketType.IDENTIFICATION_HEADER.getType() && vorbis.equals(CAPTURE_PATTERN)) {
            vorbisVersion = b[7] + (b[8] << 8) + (b[9] << 16) + (b[10] << 24);
            LOG.log(DEBUG, "vorbisVersion" + vorbisVersion);
            audioChannels = u(b[FIELD_AUDIO_CHANNELS_POS]);
            LOG.log(DEBUG, "audioChannels" + audioChannels);
            audioSampleRate = u(b[12]) + (u(b[13]) << 8) + (u(b[14]) << 16) + (u(b[15]) << 24);
            LOG.log(DEBUG, "audioSampleRate" + audioSampleRate);
            LOG.log(DEBUG, "audioSampleRate" + b[12] + " " + b[13] + " " + b[14]);

            //TODO is this right spec says signed
            bitrateMinimal = u(b[16]) + (u(b[17]) << 8) + (u(b[18]) << 16) + (u(b[19]) << 24);
            bitrateNominal = u(b[20]) + (u(b[21]) << 8) + (u(b[22]) << 16) + (u(b[23]) << 24);
            bitrateMaximal = u(b[24]) + (u(b[25]) << 8) + (u(b[26]) << 16) + (u(b[27]) << 24);
            //byte blockSize0 = (byte) ( b[28] & 240 );
            //byte blockSize1 = (byte) ( b[28] & 15 );

            int framingFlag = b[FIELD_FRAMING_FLAG_POS];
            LOG.log(DEBUG, "framingFlag" + framingFlag);
            if (framingFlag != 0) {
                isValid = true;
            }
