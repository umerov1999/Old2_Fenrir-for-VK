...
public class WavTagWriter {
    // Logger Object
    private static final JLogger LOG = JLoggers.get(WavTagWriter.class, EalvaTagLog.MARKER);
    //For logging
     ...
     * @throws CannotWriteException
     */
    private ChunkHeader seekToStartOfListInfoMetadata(FileChannel fc, WavTag existingTag)
     ...
     * @throws CannotWriteException
     */
    private ChunkHeader seekToStartOfId3Metadata(FileChannel fc, WavTag existingTag)
            throws IOException, CannotWriteException {
        ...
        if (!WavChunkType.ID3.getCode().equals(chunkHeader.getID())) {
            throw new CannotWriteException(
                    loggingName + " Unable to find ID3 chunk at original location has file been modified externally");
        }
        return chunkHeader;
    }

    public void delete(Tag tag, FileChannel fc) throws CannotWriteException {
        LOG.log(INFO, loggingName + " Deleting metadata from file");
        try {
                ...
                if (fs.isContiguous) {
                    ...
                    if (fs.isAtEnd) {
                        if (fs.isInfoTagFirst) {
                            LOG.log(INFO, loggingName + ":Setting new length to:" +
                                    existingTag.getInfoTag().getStartLocationInFile());
                    ...
                    //Slower
                        ...
                        if (fs.isInfoTagFirst) {
                            int lengthTagChunk = (int) (existingTag.getEndLocationInFileOfId3Chunk() -
                                    existingTag.getInfoTag().getStartLocationInFile());
                            deleteTagChunk(fc, (int) existingTag.getEndLocationInFileOfId3Chunk(), lengthTagChunk);
                        } else {
                            int lengthTagChunk =
                                    (int) (existingTag.getInfoTag().getEndLocationInFile().intValue() -
                                            existingTag.getStartLocationInFileOfId3Chunk());
                            deleteTagChunk(fc,
                                    existingTag.getInfoTag().getEndLocationInFile().intValue(),
                                    lengthTagChunk);
                        }
                    }
                }
            ...
            //Delete Info if exists
                ...
                if (existingInfoTag.getEndLocationInFile() == fc.size()) {
                    LOG.log(INFO, loggingName + ":Setting new length to:" + existingInfoTag.getStartLocationInFile());
                    fc.truncate(existingInfoTag.getStartLocationInFile());
                } else {
                ...
                if (isID3TagAtEndOfFileAllowingForPaddingByte(existingTag, fc)) {
                    LOG.log(INFO, loggingName + ":Setting new length to:" + existingTag.getStartLocationInFileOfId3Chunk());
                    fc.truncate(existingTag.getStartLocationInFileOfId3Chunk());
                } else {
     ...
     * @throws IOException
     */
    private void deleteInfoTagChunk(FileChannel fc, WavTag existingTag, ChunkHeader chunkHeader)
            throws IOException {
        WavInfoTag existingInfoTag = existingTag.getInfoTag();
        int lengthTagChunk = (int) chunkHeader.getSize() + ChunkHeader.CHUNK_HEADER_SIZE;
     ...
     * @throws IOException
     */
    private void deleteId3TagChunk(FileChannel fc, WavTag existingTag, ChunkHeader chunkHeader)
            throws IOException {
        int lengthTagChunk = (int) chunkHeader.getSize() + ChunkHeader.CHUNK_HEADER_SIZE;
        deleteTagChunk(fc, (int) existingTag.getEndLocationInFileOfId3Chunk(), lengthTagChunk);
    }
     ...
     * @throws IOException
     */
    private void deleteTagChunk(FileChannel fc, int endOfExistingChunk, int lengthTagChunk)
            throws IOException {
        ...
        LOG.log(DEBUG, loggingName + " Setting new length to:" + newLength);
     ...
     * @throws CannotWriteException
     */
    public void write(Tag tag, FileChannel fc) throws CannotWriteException {
        LOG.log(DEBUG, loggingName + " Writing tag to file:start");
    ...
    private void rewriteRiffHeaderSize(FileChannel fc) throws IOException {
        fc.position(SIGNATURE_LENGTH);
        ByteBuffer bb = ByteBuffer.allocateDirect(SIZE_LENGTH);
    ...
    private void writeInfoDataToFile(FileChannel fc, ByteBuffer bb, long chunkSize) throws IOException {
        ...
        //Write LIST header
        ByteBuffer listHeaderBuffer = ByteBuffer.allocate(ChunkHeader.CHUNK_HEADER_SIZE);
    ...
    private void writeID3DataToFile(FileChannel fc, ByteBuffer bb) throws IOException {
        ...
        ByteBuffer listBuffer = ByteBuffer.allocate(ChunkHeader.CHUNK_HEADER_SIZE);
    ...
    public ByteBuffer convertInfoChunk(WavTag tag) throws UnsupportedEncodingException {
        try {
            ...
            for (TagField nextField : fields) {
                ...
                LOG.log(DEBUG, loggingName + " Writing:" + wii.getCode() + ":" + next.getContent());
                ...
                if (wii == WavInfoIdentifier.TRACKNO) {
                    if (TagOptionSingleton.getInstance().isWriteWavForTwonky()) {
                        ...
                        LOG.log(DEBUG, loggingName + " Writing:" + WavInfoIdentifier.TWONKY_TRACKNO.getCode() + ":" +
                                next.getContent());
            ...
            while (ti.hasNext()) {
                ...
                LOG.log(DEBUG, loggingName + " Writing:" + next.getId() + ":" + next.getContent());
                byte[] contentConvertedToBytes = next.getContent().getBytes(StandardCharsets.UTF_8);
                baos.write(Utils.getSizeLEInt32(contentConvertedToBytes.length));
                baos.write(contentConvertedToBytes);
            ...
            ByteBuffer infoHeaderBuffer = ByteBuffer.allocate(SIGNATURE_LENGTH);
    ...
    private BothTagsFileStructure checkExistingLocations(WavTag wavTag, FileChannel fc) throws IOException {
        ...
        if (wavTag.getInfoTag().getStartLocationInFile() < wavTag.getID3Tag().getStartLocationInFile()) {
            ...
            //Must allow for odd size chunks
            if (Math.abs(wavTag.getID3Tag().getEndLocationInFile() - wavTag.getInfoTag().getStartLocationInFile()) <=
                    1) {
                fs.isContiguous = true;
     ...
     * @throws IOException
     */
    private void writeInfoChunk(FileChannel fc, WavInfoTag existingInfoTag, ByteBuffer newTagBuffer)
    ...
    private boolean isID3TagAtEndOfFileAllowingForPaddingByte(WavTag existingTag, FileChannel fc) throws IOException {
        return ((existingTag.getID3Tag().getEndLocationInFile() == fc.size()) ||
                (((existingTag.getID3Tag().getEndLocationInFile() & 1) != 0) &&
    ...
    private boolean isInfoTagAtEndOfFileAllowingForPaddingByte(WavTag existingTag, FileChannel fc) throws IOException {
        return ((existingTag.getInfoTag().getEndLocationInFile() == fc.size()) ||
                (((existingTag.getInfoTag().getEndLocationInFile() & 1) != 0) &&
                        existingTag.getInfoTag().getEndLocationInFile() + 1 == fc.size()));
    }
     ...
     * @throws IOException
     */
    private void saveBoth(WavTag wavTag, FileChannel fc, WavTag existingTag)
            throws CannotWriteException, IOException {
        //Convert both tags and get existing ones
        ByteBuffer infoTagBuffer = convertInfoChunk(wavTag);
        ByteBuffer id3TagBuffer = convertID3Chunk(wavTag, existingTag);
     ...
     * @throws IOException
     */
    private void saveActive(WavTag wavTag, FileChannel fc, WavTag existingTag)
            throws CannotWriteException, IOException {
        //Info is Active Tag
     ...
     * @throws IOException
     */
    private void saveActiveExisting(WavTag wavTag, FileChannel fc, WavTag existingTag)
            throws CannotWriteException, IOException {
        if (wavTag.getActiveTag() instanceof WavInfoTag) {
            if (existingTag.isExistingId3Tag()) {
                saveBoth(wavTag, fc, existingTag);
    ...
    /**
     * If Info/ID3 Metadata tags are corrupted and only metadata tags later in the file then just truncate metadata
     * tags and start again
     *
     * @param fc
     * @param existingTag
     * @throws IOException
     */
    private void deleteExistingMetadataTagsToEndOfFile(FileChannel fc, WavTag existingTag)
            throws IOException {
        ...
        if (!Utils.isOddLength(precedingChunk.getEndLocation())) {
            LOG.log(ERROR, loggingName + " Truncating corrupted metadata tags from:" +
                    (existingTag.getInfoTag().getStartLocationInFile() - 1));
