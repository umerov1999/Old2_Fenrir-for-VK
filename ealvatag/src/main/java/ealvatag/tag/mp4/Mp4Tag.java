...
public class Mp4Tag extends AbstractTag {
    ...
    static {
        ImmutableMap.Builder<FieldKey, Mp4FieldKey> builder = ImmutableMap.builder();
        builder.put(FieldKey.ACOUSTID_FINGERPRINT, Mp4FieldKey.ACOUSTID_FINGERPRINT)
                ...
                .put(FieldKey.DISC_NO, DISCNUMBER)
                .put(FieldKey.DISC_SUBTITLE, Mp4FieldKey.DISC_SUBTITLE)
                .put(FieldKey.DISC_TOTAL, DISCNUMBER)
                ...
                .put(FieldKey.TRACK, TRACK)
                .put(FieldKey.TRACK_TOTAL, TRACK)
    ...
    private Mp4FieldKey getMp4FieldKey(FieldKey genericKey) throws UnsupportedFieldException {
    ...
    public String getFirst(Mp4FieldKey mp4Key) throws IllegalArgumentException {
        ...
        return getFirst(mp4Key.getFieldName());
    }
    ...
    public Mp4TagField getFirstField(Mp4FieldKey mp4Key) throws IllegalArgumentException {
        checkArgNotNull(mp4Key);
        return (Mp4TagField) getFirstField(mp4Key.getFieldName()).orNull();
    }
    ...
    public Tag deleteField(FieldKey genericKey) throws IllegalArgumentException, UnsupportedFieldException {
        ...
        if (genericKey == FieldKey.KEY) {
            ...
            String trackTotal = getFirst(FieldKey.TRACK_TOTAL);
            ...
            String track = getFirst(FieldKey.TRACK);
            if (track.length() == 0) {
                deleteField(mp4FieldName);
                return this;
            } else {
            ...
            String discTotal = getFirst(FieldKey.DISC_TOTAL);
            ...
            String discno = getFirst(FieldKey.DISC_NO);
            if (discno.length() == 0) {
                deleteField(mp4FieldName);
                return this;
            } else {
            ...
            deleteField(GENRE.getFieldName());
            deleteField(GENRE_CUSTOM.getFieldName());
        } else {
            deleteField(mp4FieldName);
        }
        return this;
    ...
    public void deleteField(Mp4FieldKey mp4Key) throws IllegalArgumentException {
        checkArgNotNull(mp4Key);
        deleteField(mp4Key.getFieldName());
    }
    ...
    private static class TagFieldSupplier implements Supplier<TagField> {
        private final Mp4TrackField mp4TrackField = new Mp4TrackField(0);

        @Override
        public TagField get() {
            return mp4TrackField;
        }
    }

    private static class DiscTagFieldSupplier implements Supplier<TagField> {
        private final Mp4DiscNoField mp4DiscNoField = new Mp4DiscNoField(0);
