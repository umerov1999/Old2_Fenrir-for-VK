...
public class Mp4DataBox extends AbstractMp4Box {
    ...
    public Mp4DataBox(Mp4BoxHeader boxHeader, ByteBuffer raw) {
        ...
        //Type
        type = Utils.getIntBE(dataBuffer, TYPE_POS, TYPE_POS + TYPE_LENGTH - 1);

        if (type == Mp4FieldType.TEXT.getFileClassId()) {
            content = Utils.getString(dataBuffer, PRE_DATA_LENGTH, boxHeader.getDataLength() - PRE_DATA_LENGTH, boxHeader.getEncoding());
            ...
            for (int i = 0; i < ((boxHeader.getDataLength() - PRE_DATA_LENGTH) / NUMBER_LENGTH); i++) {
                numbers.add(Utils.getShortBE(dataBuffer,
            ...
            content = Integer.toString(Utils.getIntBE(dataBuffer, PRE_DATA_LENGTH, boxHeader.getDataLength() - 1));
     ...
     * :data
     */
    public Mp4DataBox(Mp4BoxHeader dataBoxHeader, Buffer buffer) throws IOException {
        ...
        int remainingDataSize = header.getDataLength() - PRE_DATA_LENGTH;
        if (type == Mp4FieldType.TEXT.getFileClassId()) {
            content = buffer.readString(remainingDataSize, header.getEncoding());
        } else if (type == Mp4FieldType.IMPLICIT.getFileClassId()) {
            numbers = new ArrayList<>();

            int shortsToRead = (remainingDataSize) / NUMBER_LENGTH;
            ...
            //Create String representation for display
            content = Integer.toString(Utils.getIntBE(dataBuffer, PRE_DATA_LENGTH, header.getDataLength() - 1));
            ...
            content = Utils.getString(dataBuffer, PRE_DATA_LENGTH, remainingDataSize, header.getEncoding());
        }
    }
